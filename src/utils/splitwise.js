...
function downheapify(heap, idx) {
	...
	var largest = idx;
	if(heap[lc].first > heap[largest].first) {
		largest = lc;
	}
	if(heap[rc].first > heap[largest].first) {
		largest = rc;
	}
...
export function splitwise(transactions) {
	...
	for(const p in net_balance) {
		if(net_balance[p] > 0) {
			...
			push_heap(negative, -net_balance[p], p);
		}
	}

	var result = [];

	while(positive.length > 0) { // positve goes along with negative
		var p1 = heap_top(positive);
		var p2 = heap_top(negative);
		pop_heap(positive);
		pop_heap(negative);
		let min = Math.min(p1.first,p2.first);
		let exp = new Expense(p2.second, p1.second, Math.min(p1.first, p2.first));
		result.push(exp);
		if(!(p1.first == min))
		{
			push_heap(positive, p1.first - min, p1.second);
		}
		else if(!(p2.first == min))
		{
			push_heap(negative, p2.first - min, p2.second);
		}
